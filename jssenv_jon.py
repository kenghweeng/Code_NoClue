...
class JssEnv(gym.Env):
    ...
    def __init__(self, env_config=None):
        ...
        self.start_timestamp = datetime.datetime.fromtimestamp(-27000).timestamp() # datetime.datetime.now().timestamp()
        self.sum_op = 0
    ...
    def render(self, mode='human', machine2label=None):
        ...
        for job in range(self.jobs):
            for i in range(self.machines):
                ...
                dict_op["Start"] = self.start_timestamp-1e-9
                dict_op["Finish"] = self.start_timestamp-1e-9
        ...
        if len(df) > 0:
            df = pd.DataFrame(df)
            df['Start'] = df['Start'].apply(datetime.datetime.fromtimestamp)
            df['Finish'] = df['Finish'].apply(datetime.datetime.fromtimestamp)
